...
class BaseModel(nn.Module):
    ...
    def _apply(self, fn):
        ...
        if isinstance(m, (PDetect,Detect)):  # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
...
#             self.stride = torch.Tensor([32])  # default stride for i.e. RTDETR
        m = self.model[-1]  # Detect()
        if isinstance(m, (Detect,PDetect)): # includes all Detect subclasses like Segment, Pose, OBB, WorldDetect
            s = 256  # 2x min stride
            m.inplace = self.inplace
            forward = lambda x: self.forward(x)[0] if isinstance(m, (Segment, Pose, OBB)) else self.forward(x)
...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            if m in {BottleneckCSP, C1, C2, C2f, C2fAttn, C3, C3TR, C3Ghost, C3x, RepC3,C2f_repghost,ShuffleAttention,CP4}:
